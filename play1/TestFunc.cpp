...
void testFunc32(int argc, char* argv[])
{
	std::cout << "test func 32" << std::endl;
	// parameters of NN
	int num_threads = atoi(argv[1]);
	int batch_size = BATCH_SIZE;
	// int num_src_terms = NUM_SRC_TERMS;
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic) shared(args, src_embd)
	for (int i = 0; i < batch_size; i ++)
	{
		...
		for (int j = 0; j < num_tgt_terms[i]; j ++)
		{
			// softmaxOperation(args[id]->decStates[j]->h, args[id]->target_dist, tgt_data[id][j], args[id]->decStates[j]->delh, softmax, softmax_2);
			//softmaxOperation(args[id]->decStates[j]->h, tgt_data[id][j], args[id]->decStates[j]->delh, softmax, args[id]->softmax);
		}
	...
	return;
} // end of test function 32

void testFunc33(int argc, char* argv[])
{
	std::cout << "test func 33" << std::endl;
	// parameters of NN
	int num_threads = atoi(argv[1]);
	int batch_size = BATCH_SIZE;
	// int num_src_terms = NUM_SRC_TERMS;
